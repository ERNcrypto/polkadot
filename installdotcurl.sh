...
sudo chmod -R 777 /home/$current_user/.local/share/polkadot/chains/polkadot/db/full

sudo chmod -R 777 /home/$current_user/.local/share/polkadot/chains/polkadot/
...
Type=simple
User=$current_user
ExecStart=$HOME/polkadot-sdk/target/release/polkadot --validator --name "$STARTNAME" --chain=polkadot --database RocksDb --telemetry-url 'wss://telemetry-backend.w3f.community/submit 1' --state-pruning 1000 --prometheus-external --prometheus-port=9615 --insecure-validator-i-know-what-i-do --unsafe-force-node-key-generation
Restart=always
RestartSec=10
...
sudo curl -o - -L https://snapshots.radiumblock.com/polkadot_6247335_2024-10-16.tar.lz4 | lz4 -c -d - | sudo tar -x -C /home/$current_user/.local/share/polkadot/chains/polkadot/
...
Type=simple
User=$current_user
ExecStart=$HOME/polkadot-sdk/target/release/polkadot --validator --name "$STARTNAME1" --chain=polkadot --database RocksDb --telemetry-url 'wss://telemetry-backend.w3f.community/submit 1' --state-pruning 1000 --prometheus-external --prometheus-port=9615 --insecure-validator-i-know-what-i-do
Restart=always
RestartSec=10
StandardOutput=journal
StandardError=journal
